...
<Modal {open} {onclose} title="Reassign owner" subtitle="Hand this lead to another rep." width={420}>
	<div class="flex flex-col gap-1.5">
		{#each reps as u (u.id)}
			<Button
				variant="outline"
				class="h-11 justify-start gap-2.5 {selected === u.id ? 'border-primary bg-[rgba(192,54,44,0.06)]' : ''}"
