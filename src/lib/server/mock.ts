...
export const MOCK_LEADS: MockLead[] = [
	...
	{
		id: 'l-7',
		name: 'Unknown Org (recovered row)',
		category: 'Other',
		platform: 'Facebook',
		location: '',
		stage: 'contacted',
...
export const MOCK_LEADERBOARD = MOCK_REPS.filter((r) => r.active && r.role === 'rep').map(
	(r, i) => ({
		rep: r.name,
		wins: [1, 1, 0, 1, 0][i] ?? 0,
		touches: [210, 180, 150, 140, 90][i] ?? 0,
		replies: [60, 50, 40, 35, 20][i] ?? 0
	})
);
...
export const MOCK_ACTIVITIES: MockActivity[] = [
	...
	},
	{
		...
		notes: 'They asked about pricing.'
	},
	{
		id: 'a-3',
		leadId: 'l-5',
