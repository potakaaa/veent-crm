	...
	{#each groupDefs as g (g.key)}
		...
		<div class="mb-5">
			<div class="mb-2.5 flex flex-wrap items-center gap-x-2.5 gap-y-1">
				...
				<h2 class="whitespace-nowrap font-serif text-[15px] font-semibold text-ink">{g.title}</h2>
				{#if navLoading}
					<span class="inline-block h-4 w-5 shrink-0 animate-pulse rounded bg-muted"></span>
				{:else}
					<span
