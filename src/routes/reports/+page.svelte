<script lang="ts">
	...
	import type { CurrencyTotal, FunnelStage, LeaderboardRow } from '$lib/types';
...
<div class="px-7 pb-16 pt-6">
	...
	>
		{#snippet actions()}
			<button class="h-[34px] rounded-control border border-hairline bg-panel px-3 font-mono text-[12.5px] text-ink-600">
				1 Mar – 24 Jun 2026 ▾
			</button>
		{/snippet}
	</PageHeader>
	...
	<div class="mb-[18px] grid grid-cols-1 gap-[18px] lg:grid-cols-[1.1fr_1fr]">
			...
			{#each report.funnel as f (f.stage)}
				<div class="mb-3">
					...
					<div class="h-[22px] overflow-hidden rounded-[5px] bg-panel-sunken">
						<div class="h-full rounded-[5px]" style="width:{Math.round((f.count / maxCount) * 100)}%;background:{f.color}"></div>
					</div>
				</div>
			{/each}
		</div>
		...
		<div class="rounded-control border border-hairline bg-panel p-5">
			...
			<div class="grid grid-cols-[1.4fr_1fr_1fr_0.8fr] gap-2 border-b border-hairline pb-2.5 font-mono text-[10px] uppercase tracking-[0.4px] text-ink-300">
				<span>Rep</span><span class="text-right">Touches</span><span class="text-right">Replies</span><span class="text-right">Wins</span>
			</div>
			{#each report.leaderboard as r (r.repId)}
				<div class="grid grid-cols-[1.4fr_1fr_1fr_0.8fr] items-center gap-2 border-b border-panel-sunken py-2.5 last:border-b-0">
					<div class="flex items-center gap-2"><Avatar name={r.name} /><span class="text-[13px] font-semibold">{r.name}</span></div>
	...
	<div class="rounded-control border border-hairline bg-panel p-5">
		<div class="mb-3.5 flex items-center justify-between">
			...
			<span class="text-[11.5px] text-ink-200">manually captured in Veent · never read from external systems</span>
		</div>
		<div class="flex flex-wrap gap-4">
			{#each report.currencyTotals as c (c.currency)}
				<div class="min-w-[200px] flex-1 rounded-control border border-hairline bg-[#fdf7f5] p-4">
					...
					<div class="mt-1 font-mono text-[26px] font-semibold tracking-[-1px] tnum">{formatMoney(c.total, c.currency)}</div>
