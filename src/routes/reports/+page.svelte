...
<StubNote
	>Apache ECharts funnel + leaderboard + CSV export go here (bars below are placeholders).</StubNote
>
	...
	<section class="rounded-lg border border-gray-200 bg-white p-4">
		...
		<table class="w-full text-sm">
			<thead class="text-left text-gray-500">
				<tr
					><th class="py-1 font-medium">Rep</th><th class="py-1 font-medium">Wins</th><th
						class="py-1 font-medium">Touches</th
					><th class="py-1 font-medium">Replies</th></tr
				>
			</thead>
			<tbody class="divide-y divide-gray-100">
				{#each data.leaderboard as r}
					<tr
						><td class="py-1.5 font-medium">{r.rep}</td><td>{r.wins}</td><td>{r.touches}</td><td
							>{r.replies}</td
						></tr
					>
				{/each}
		...
		<p class="mt-3 text-xs text-gray-400">
			Wins + activity shown side-by-side so cherry-picking is visible.
		</p>
	</section>
</div>
